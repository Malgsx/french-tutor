...
- Captions/history are memory-only by default and cleared on session end. Opt-in
  ...
  **Review saved transcripts** groups recordings by local day, newest first,
  and opens each one as a chat-style document (`Me:` / `Miette:`). Records
  saved before session tracking appear under “Earlier recordings”. Turning
  retention off deletes saved transcripts immediately.
